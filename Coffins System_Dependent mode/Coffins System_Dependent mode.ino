// Greenhouse Coffens script-DEPENDENT MODE
...
void loop()
{
  ...
  float OpenOrClose; 
  if(adc_value >200) {
    OpenOrClose =1; // Flow is detected, set OpenOrClose to 1 (1 for closed, 0 for open)
  }else{
      OpenOrClose =0; // No flow detected, set OpenOrClose to 0 (1 for closed, 0 for open)
    }
 ...
 if(adc_value > 200) {
    ...
    Serial.print(";");
    Serial.print(OpenOrClose); // Indicator for the coffen being open (1 for open, 0 for closed)
    Serial.print(";");
    Serial.print(t); // Print temperature to the Serial monitor
    Serial.print(";");
    Serial.print(h); // Print temperature to the Serial monitor
    Serial.print(";");
    Serial.println(adc_value); // Print humidity to the Serial monitor
    ...
    if (dataFile) {
      ...
      dataFile.print(";");
      dataFile.println(OpenOrClose); // Write the indicator for coffen open to the file
  ...
  delay(Measurement_freq); 
  }// Delay for 5 seconds between iterations
    ...
    Serial.print(timestring); // Print the formatted timestamp to the Serial monitor
    Serial.print(";");
    Serial.print(OpenOrClose); // Indicator for the coffen being closed (1 for open, 0 for closed)
    Serial.print(";");
    Serial.print(t); // Print temperature to the Serial monitor
    Serial.print(";");
    Serial.print(h); // Print temperature to the Serial monitor
  ...
  //Serial.print(";"); 
  delay(Measurement_freq);  // Delay for 5 seconds between iterations
  }
